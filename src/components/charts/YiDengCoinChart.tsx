...
const YiDengCoinChart = () => {
  ...
  useEffect(() => {
    ...
    const generateMockData = (): CoinDataPoint[] => {
      ...
      while (currentDay <= today) {
        if (currentDay.getDay() !== 0 && currentDay.getDay() !== 6) { // 排除周末
          // 创建一些波动性以增加真实感
          const dayProgress = (currentDay.getTime() - startDate.getTime()) / (today.getTime() - startDate.getTime());
          ...
          // 随机生成开盘价（前一天收盘价附近）
          const open = mockData.length === 0 ? price + (Math.random() * 10) : mockData[mockData.length-1].price;
      ...
      if (mockData.length > 0) {
        ...
        mockData[mockData.length - 1].low = 270.00;
      ...
      return (
        <div className="cyberpunk-card p-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2">
            <p className="text-gray-400">{t('chart.open')}: <span className="text-white">${tooltipData.open}</span></p>
            <p className="text-gray-400">{t('chart.close')}: <span className="text-white">${tooltipData.price}</span></p>
            <p className="text-green-400">{t('chart.high')}: <span className="text-white">${tooltipData.high}</span></p>
            <p className="text-red-400">{t('chart.low')}: <span className="text-white">${tooltipData.low}</span></p>
            <p className="text-gray-400">{t('chart.volume')}: <span className="text-white">{tooltipData.volume.toLocaleString()}</span></p>
            <p className="text-yellow-400">
              {t('chart.change')}: 
              <span className={tooltipData.price > tooltipData.open ? "text-green-400 ml-1" : "text-red-400 ml-1"}>
  ...
  const getChartModeText = (): string => {
    switch(viewMode) {
      case 'area': return '成交量';
      case 'volume': return 'K线图';
      case 'candle': return 'OHLC图';
      default: return '区域图';
    }
  };
  ...
  return (
    <div className="cyberpunk-card p-2 h-100 relative overflow-hidden" style={{ backgroundColor: 'rgba(10, 11, 30, 0.9)', borderRadius: '12px', border: '1px solid #05d9e8' }}>
      <div className="flex justify-between items-center mb-2">
        <h3 className="cyberpunk-title text-xl" style={{ 
        ...
        {crosshairValues ? (
          <div className="flex justify-between">
            ...
            <span className="text-neon-green">
              24h: <span className="text-red-400">
                -12.43%
              </span>
            </span>
          </div>
      ...
      <div ref={chartRef} style={{ backgroundColor: '#0a0b1e', borderRadius: '8px', height:'90%' }}>
